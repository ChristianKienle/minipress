...
const { removeTrailingSlash, stringify } = require('@minipress/utils')
...
// options stolen from nuxt – minimizing more aggressivly might cause routing issues
const minifyHtml = html => htmlMinifier(html, {
...
module.exports = class VueRenderer {
  ...
  async generatePage({ page, renderer, outDir }) {
    ...
    this.log.info(`${path} generated (${minifiedHtml.length} bytes)`)
    const outPath = outputPath
    await fs.outputFile(outPath, minifiedHtml)
  }
  ...
  async getRequestHandler({ dest }) {
    ...
    server.get('*', async (
      ...
      const render = async () => {
        ...
        const url = removeTrailingSlash(replacePrefix(req.url, this.config.build.base, '/'))
