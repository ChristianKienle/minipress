// @ts-check
const MinpressWebpackConfig = require('./webpack-config')
 ...
 * @prop {import('@minpress/log').Logger} log
 */
class Minpress {
  /** @param {Options} options */
  constructor({ config, log }) {
    this.config = config
    this.webpack = new MinpressWebpackConfig({ config, log })
    this.log = log
  ...
  _init() {
    ...
    enableMarkdownSupport({ webpack: this.webpack, minpress: this })

    const aliases = ['pages', 'layouts', 'routes', 'components', 'site-data']
    aliases.forEach(alias => {
      const name = `#minpress/${alias}`
      ...
      // ['pages', 'pages/index.js'],
      // ['pages', 'pages/index.js'],
    // this.alias('#minpress/pages', this.tempDir.writeTemp('pages/index.js', ''))
    // this.alias('#minpress/layouts', this.tempDir.writeTemp('layouts/index.js', ''))
    // this.alias('#minpress/components', this.tempDir.writeTemp('components/index.js', ''))
    // this.alias('#minpress/routes', this.tempDir.writeTemp('routes/index.js', ''))
    // this.alias('#minpress/site-data', this.tempDir.writeTemp('site-data/index.js', ''))

    // Dynamic Modules
    Object.entries(this.config.dynamicModules({ context: this })).map(([name, code]) => {
      const moduleName = `#minpress/dynamic/${name}`
